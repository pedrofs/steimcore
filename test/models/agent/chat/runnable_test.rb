...
class Agent::Chat::RunnableTest < ActiveSupport::TestCase
  ...
  test "run_turn! never reshapes the persisted history; healing happens read-only at send time" do
    # Reproduce a broken-chat shape: an extra empty user + a trailing duplicate
    # user, on top of the user message created in setup. None of these rows
    # should be mutated or deleted by a turn — the send-time selection layer
    # (Agent::Chat::MessageSelection) handles validity without touching the DB.
    @chat.messages.create!(role: :user, content: "",      trainer: @user)
    @chat.messages.create!(role: :user, content: "extra", trainer: @user)

    behavior = ->(chat, &_block) { chat.messages.create!(role: :assistant, content: "ok") }

    assert_no_difference -> { @chat.messages.where(role: :user).count } do
      stub_agent_new(behavior) { @chat.run_turn! }
    end

    contents = @chat.reload.messages.where(role: :user).order(:created_at).pluck(:content)
    assert_equal [ "Olá", "", "extra" ], contents, "persisted user rows must be left exactly as written"
  end
